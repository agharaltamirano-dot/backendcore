    ...
    private static string[] Values(Horario h)
{
    ...
    private static void ExcelHeader(IXLWorksheet s, string? usuario, List<string> filtros) { s.Range("A1:F2").Merge(); s.Cell("A1").Value = "Reporte detallado de horarios"; s.Cell("A1").Style.Font.Bold = true; s.Cell("A1").Style.Font.FontSize = 16; var p = Path.Combine(Directory.GetCurrentDirectory(), "assets", "logo3.jpeg"); if (System.IO.File.Exists(p)) s.AddPicture(p).MoveTo(s.Cell("J1")).WithSize(110, 70); s.Range("H3:J3").Merge(); s.Range("H4:J4").Merge(); s.Cell("H3").Value = $"Generado por: {usuario ?? "No especificado"}"; s.Cell("H4").Value = $"Fecha de generación: {DateTime.Now:dd/MM/yyyy HH:mm}"; s.Range("H3:J4").Style.Alignment.Horizontal = XLAlignmentHorizontalValues.Right; s.Range("A5:J5").Merge(); s.Cell("A5").Value = filtros.Count == 0 ? "Criterios: sin filtros" : "Criterios: " + string.Join(" | ", filtros); s.Cell("A5").Style.Alignment.WrapText = true; }
