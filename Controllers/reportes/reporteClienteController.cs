...
public class reporteClienteController : ControllerBase
{
    ...
    public async Task<IActionResult> ExportarPdf([FromQuery] bool? estado, [FromQuery] string? ci, [FromQuery] string? nombreCompleto, [FromQuery] string? telefono, [FromQuery] string? nombreUsuario)
    {
        ...
        void Page() { g?.Dispose(); var p = doc.AddPage(); p.Size = PdfSharpCore.PageSize.Letter; p.Orientation = PdfSharpCore.PageOrientation.Landscape; g = XGraphics.FromPdfPage(p); w = p.Width.Point; h = p.Height.Point; y = m; g.DrawString("Reporte detallado de clientes", title, XBrushes.Black, new XRect(m, y, 400, 22), XStringFormats.TopLeft); if (System.IO.File.Exists(logoPath)) { using var logo = XImage.FromFile(logoPath); g.DrawImage(logo, w - m - 62, y, 62, 42); } y += 24; g.DrawString($"Generado por: {nombreUsuario ?? "No especificado"}", meta, XBrushes.Black, new XRect(w - m - 250, y, 250, 12), XStringFormats.TopRight); g.DrawString($"Fecha de generación: {DateTime.Now:dd/MM/yyyy HH:mm}", meta, XBrushes.Black, new XRect(w - m - 250, y + 11, 250, 12), XStringFormats.TopRight); y += 27; g.DrawString(r.Filtros.Count == 0 ? "Criterios: sin filtros" : "Criterios: " + string.Join(" | ", r.Filtros), meta, XBrushes.Black, new XRect(m, y, w - m * 2, 20), XStringFormats.TopLeft); y += 22; TableHeader(); }
        Page(); foreach (var c in r.Clientes) { if (y + 20 > h - m) Page(); var x = m; var v = Values(c); foreach (var col in Cols()) { g!.DrawRectangle(XPens.LightGray, x, y, col.W, 20); g.DrawString(v[col.I], text, XBrushes.Black, new XRect(x + 3, y + 3, col.W - 6, 14), XStringFormats.TopLeft); x += col.W; } y += 20; } g?.Dispose(); using var ms = new MemoryStream(); doc.Save(ms, false); Response.Headers["Content-Disposition"] = "inline; filename=\"reporte_clientes.pdf\""; return File(ms.ToArray(), "application/pdf");
    }
    ...
    public async Task<IActionResult> ExportarXlsx([FromQuery] bool? estado, [FromQuery] string? ci, [FromQuery] string? nombreCompleto, [FromQuery] string? telefono, [FromQuery] string? nombreUsuario)
    {
        ...
        s.Range("A1:C2").Merge(); s.Cell("A1").Value = "Reporte detallado de clientes"; s.Cell("A1").Style.Font.Bold = true; s.Cell("A1").Style.Font.FontSize = 16; var path = Path.Combine(Directory.GetCurrentDirectory(), "assets", "logo9.png"); if (System.IO.File.Exists(path)) s.AddPicture(path).MoveTo(s.Cell("E1")).WithSize(110, 70);
