...
public class reportePasajesController : ControllerBase
{
    ...
    [HttpGet("reporte-pasajes/json")]
    [AllowAnonymous]
    public async Task<IActionResult> ExportarPasajesJson(
        [FromQuery] int? clienteId,
        [FromQuery] int? asientoId,
        [FromQuery] string? destino,
        [FromQuery] bool? estado,
        [FromQuery] string? fechaDesde,
        [FromQuery] string? fechaHasta,
        [FromQuery] string? movil,
        [FromQuery] int? usuarioId,
        [FromQuery] bool? reserva,
        [FromQuery] string? nombreUsuario)
    {
        try
        {
            // Reutilizas el mismo método que PDF
            var r = await Consultar(clienteId, asientoId, destino, estado,
                                    fechaDesde, fechaHasta, movil, usuarioId, reserva);

            if (r.Error != null)
                return BadRequest(r.Error);

            // Devuelves directamente la lista de pasajes
            return Ok(new { pasajes = r.Pasajes });
        }
        catch (Exception ex)
        {
            Console.WriteLine($"[ERROR] reporte-pasajes/json: {ex}");
            return StatusCode(500, "Error interno al generar el reporte JSON de pasajes.");
        }
    }
    ...
    private async Task<Result> Consultar(int? clienteId, int? asientoId, string? destino, bool? estado, string? fechaDesde, string? fechaHasta, string? movil, int? usuarioId, bool? reserva)
    {
        ...
        {
            var cliente = await _context.Clientes.AsNoTracking().FirstOrDefaultAsync(c => c.Id == clienteId.Value);
            f.Add($"Cliente: {cliente?.NombreCompleto ?? clienteId.Value.ToString()}");
        }
        if (asientoId.HasValue) f.Add($"Asiento ID: {asientoId}");
        if (!string.IsNullOrWhiteSpace(destino)) f.Add($"Destino: {destino.Trim()}");
        if (estado.HasValue) f.Add($"Estado: {(estado.Value ? "Activo" : "Anulado")}");
        if (fechaDesde != null) f.Add($"Fecha desde: {fechaDesde}");
        if (fechaHasta != null) f.Add($"Fecha hasta: {fechaHasta}");
        if (!string.IsNullOrWhiteSpace(movil)) f.Add($"Móvil: {movil.Trim()}");
        if (usuarioId.HasValue)
        {
            var usuario = await _context.Usuarios.AsNoTracking().FirstOrDefaultAsync(u => u.Id == usuarioId.Value);
            f.Add($"Usuario: {usuario?.Usuario1 ?? usuarioId.Value.ToString()}");
        }
        if (reserva.HasValue) f.Add($"Reserva: {(reserva.Value ? "Sí" : "No")}");
